...
public class PlayerInAirState : PlayerState
{
    ...
    public override void LogicUpdate()
    {
        ...
        if (isGrounded && player.CurrentVelcoity.y < 0.01f)
        {
        ...
        else
        {
            ...
            player.Animator.SetFloat("YVelocity", player.CurrentVelcoity.y);
        }
    }
    ...
    private void CheckJumpMultiplier()
    {
        if (isJumping)
        {
            if (jumpInputStop)
            {
                player.SetVelocityY(player.CurrentVelcoity.y * playerData.variableJumpHeightMultiplier);
                isJumping = false;
            }
            else if (player.CurrentVelcoity.y <= 0)
            {
                isJumping = false;
            }
        }
